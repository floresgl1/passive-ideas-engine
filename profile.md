...
- **Applied statistical reasoning under uncertainty** *(GRADUATED to [strong] — see changelog)* — CI lower bound vs. a dynamically-computed breakeven, hit-rate-needs-payoff-ratio, sample-size intuition, leading-vs-lagging indicator separation, volume gates as necessary-but-not-sufficient. Shipped: it gates finance_bot's live behavior.
...
- **Learns by building instrumentation** *(GRADUATED out of Intent — see changelog)* — builds tooling/harnesses/simulators to serve his own understanding; proven by Claude Code sub-agents and harness-before-feature patterns in the repos, not merely stated.
